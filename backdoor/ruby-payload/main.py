...
$INTERVAL = '''+payloadInfo['interval'].strip()+'''.0;
'''
...
def create(type):
    ...
    if type == "-o":
        print(f"{colorama.Fore.GREEN}COMING SOON... instead of [create -c]{colorama.Style.RESET_ALL}")

    elif type == "-c":
        print(f"{colorama.Fore.GREEN}CREATING PAYLOAD [CODE]{colorama.Style.RESET_ALL}")
        filePath = os.path.join(outputPath, "main.rb")
